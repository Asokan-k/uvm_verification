class counter_env extends uvm_env;
  ...
  function void build_phase(uvm_phase phase);
    ...
    if(m_cfg.has_input_agent)
      input_agent_h =
        counter_input_agent::type_id::create("input_agent_h", this);

    if(m_cfg.has_output_agent)
      output_agent_h =
        counter_output_agent::type_id::create("output_agent_h", this);

    if(m_cfg.has_scoreboard)
      scoreboard_h =
        counter_scoreboard::type_id::create("scoreboard_h", this);
  ...
  function void connect_phase(uvm_phase phase);
    ...
    if(m_cfg.has_input_agent && m_cfg.has_scoreboard)
      input_agent_h.monh.monitor_port.connect(
        scoreboard_h.input_mon_fifo.analysis_export);

    if(m_cfg.has_output_agent && m_cfg.has_scoreboard)
      output_agent_h.monh.monitor_port.connect(
        scoreboard_h.output_mon_fifo.analysis_export);
