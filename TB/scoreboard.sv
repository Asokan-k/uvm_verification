class counter_scoreboard extends uvm_scoreboard;
  ...
  endgroup
    ...
    forever begin
      ...
      `uvm_info(get_type_name(),
        $sformatf("sb has got below pkt from input monitor \n%s",
        input_mon_pkt.sprint()), UVM_MEDIUM)

      output_mon_fifo.get(output_mon_pkt);
      output_mon_pkt_count++;

      `uvm_info(get_type_name(),
        $sformatf("sb has got below pkt from output monitor \n%s",
  ...
  endtask
    ...
    begin
      if (input_mon_pkt.rst ||
          (input_mon_pkt.load && input_mon_pkt.data_in >= 13))
        expected_output_pkt.data_out = 0;

      else if (input_mon_pkt.load)
        expected_output_pkt.data_out = input_mon_pkt.data_in;

      else if (expected_output_pkt.data_out <= 13)
        expected_output_pkt.data_out =
          expected_output_pkt.data_out + 1'b1;
  ...
  endtask
    ...
    if (!expected_output_pkt.compare(output_mon_pkt)) begin
      `uvm_info(get_type_name(),
        $sformatf("expected packet is below \n%s",
        expected_output_pkt.sprint()), UVM_MEDIUM)

      `uvm_info(get_type_name(),
        $sformatf("dut's output packet is below \n%s",
        output_mon_pkt.sprint()), UVM_MEDIUM)
      ...
      `uvm_info(get_type_name(),
        $sformatf("Data Match successful"), UVM_MEDIUM)
  ...
  endtask
    ...
    $display(" input mon pkt count = %0d, output mon pkt count = %0d, no_of_successful comparisons = %0d\n",
